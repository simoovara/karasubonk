...
function connectKarasu() {
  ...
  socketKarasu.onmessage = function (event) {
    ...
    if (data.type == "versionReport") {
      ...
      if (guideX == null) guideX = window.innerWidth / 2;
      if (guideY == null) guideY = window.innerHeight / 2;
        ...
        case 1:
          ...
          socketVTube.onmessage = function (event) {
            ...
            request = {
              ...
              stage: "min",
              positionX:
                tempData.modelPosition.positionX -
                ((guideX / window.innerWidth) * 2 - 1),
              positionY:
                tempData.modelPosition.positionY +
                ((guideY / window.innerHeight) * 2 - 1),
      ...
      socketVTube.onmessage = async function (event) {
        ...
        const faceWidthMin =
          data.data[modelID + "Min"] == null
            ? 0
            : data.data[modelID + "Min"][0];
        const faceHeightMin =
          data.data[modelID + "Min"] == null
            ? 0
            : data.data[modelID + "Min"][1];
        const faceWidthMax =
          data.data[modelID + "Max"] == null
            ? 0
            : data.data[modelID + "Max"][0];
        const faceHeightMax =
          data.data[modelID + "Max"] == null
            ? 0
            : data.data[modelID + "Max"][1];
        ...
        switch (data.type) {
          ...
          case "barrage":
            ...
            var queueId = 0.0;
            while (bonkQueues[queueId] != null) queueId++;
            ...
            if (i < max) {
              var bonker = setInterval(function () {
                ...
                if (++i >= max) clearInterval(bonker);
              }, data.data.barrageFrequency * 1000);
            }
            break;
          default:
            if (data.data.customBonks[data.type].barrageCountOverride)
              data.data.barrageCount =
                data.data.customBonks[data.type].barrageCount;
            if (data.data.customBonks[data.type].barrageFrequencyOverride)
              data.data.barrageFrequency =
                data.data.customBonks[data.type].barrageFrequency;
            if (data.data.customBonks[data.type].throwDurationOverride)
              data.data.throwDuration =
                data.data.customBonks[data.type].throwDuration;
            if (data.data.customBonks[data.type].throwAngleOverride) {
              data.data.throwAngleMin =
                data.data.customBonks[data.type].throwAngleMin;
              data.data.throwAngleMax =
                data.data.customBonks[data.type].throwAngleMax;
            }
            if (data.data.customBonks[data.type].throwDirectionOverride)
              data.data.throwDirection =
                data.data.customBonks[data.type].throwDirection;
            if (data.data.customBonks[data.type].spinSpeedOverride) {
              data.data.spinSpeedMin =
                data.data.customBonks[data.type].spinSpeedMin;
              data.data.spinSpeedMax =
                data.data.customBonks[data.type].spinSpeedMax;
            }
            ...
            if (windupSound != null) {
              windup = new Audio();
              windup.src =
                windupSound.location.substr(
                  0,
                  windupSound.location.indexOf("/") + 1,
                ) +
                encodeURIComponent(
                  windupSound.location.substr(
                    windupSound.location.indexOf("/") + 1,
              ...
              canPlayWindup = false;
              windup.oncanplaythrough = function () {
                canPlayWindup = true;
              };
            } else canPlayWindup = true;

            while (!canPlayWindup)
              await new Promise((resolve) => setTimeout(resolve, 10));

            if (windupSound != null) windup.play();
            ...
            setTimeout(() => {
              ...
              if (i < cMax) {
                var bonker = setInterval(function () {
                  ...
                  if (++i >= cMax) clearInterval(bonker);
                }, data.data.barrageFrequency * 1000);
              }
    ...
    if (failedAuth) {
      ...
      socketVTube.onmessage = function (event) {
        ...
        if (response.messageType == "AuthenticationTokenResponse") {
          ...
          response.messageType == "APIError" &&
          response.data.errorID == 50
        )
  ...
  if (vTubeIsOpen) {
    ...
    socketVTube.onmessage = function (event) {
      ...
      if (pos != null) {
        const offsetX =
          faceWidthMin +
          ((pos.size + 100) / 200) * (faceWidthMax - faceWidthMin);
        const offsetY =
          faceHeightMin +
          ((pos.size + 100) / 200) * (faceHeightMax - faceHeightMin);
        ...
        const angle =
          (Math.random() * (data.throwAngleMax - data.throwAngleMin) +
            data.throwAngleMin) *
          multH;
        const sizeScale =
          data.itemScaleMin +
          ((pos.size + 100) / 200) * (data.itemScaleMax - data.itemScaleMin);
        ...
        if (sound != null) {
          audio = new Audio();
          audio.src =
            sound.substr(0, sound.indexOf("/") + 1) +
            encodeURIComponent(sound.substr(sound.indexOf("/") + 1));
          audio.volume = volume * data.volume;
          canPlayAudio = false;
          audio.oncanplaythrough = function () {
            canPlayAudio = true;
          };
        } else canPlayAudio = true;

        var impact, canShowImpact;
        if (impactDecal != null) {
          impact = new Image();
          impact.src =
            "decals/" + encodeURIComponent(impactDecal.location.substr(7));
          canShowImpact = false;
          impact.onload = function () {
            canShowImpact = true;
          };
        } else canShowImpact = true;

        var img = new Image();
        if (image.startsWith("https://static-cdn.jtvnw.net/emoticons/v1/"))
          img.src = image;
        else img.src = "throws/" + encodeURIComponent(image.substr(7));
        ...
        img.onload = async function () {
          ...
          if (bonkCounts[queueId] <= 0) {
            ...
            bonkIntervals[queueId] = setInterval(
              bonkQueueTimeout,
              1000.0 / 60.0,
              queueId,
            );
          ...
          root.style.transformOrigin =
            ((pos.positionX + 1) / 2) * 100 +
            "% " +
            (1 - (pos.positionY + 1) / 2) * 100 +
            "%";
          ...
          pivot.style.left =
            window.innerWidth * xPos -
            (img.width * scale * sizeScale) / 2 +
            randH +
            "px";
          pivot.style.top =
            window.innerHeight * yPos -
            (img.height * scale * sizeScale) / 2 +
            randV +
            "px";
          ...
          if (data.spinSpeedMax - data.spinSpeedMin == 0)
            ...
            thrown.style.animationName =
              "spin" +
              (Math.random() < 0.5 ? "Clockwise " : "CounterClockwise ");
            thrown.style.animationDuration =
              3 /
                (data.spinSpeedMin +
                  Math.random() * (data.spinSpeedMax - data.spinSpeedMin)) +
              "s";
            setTimeout(
              function () {
                thrown.style.animationDuration =
          ...
          setTimeout(
            function () {
              flinch(
                ...
                angle,
                weight,
                data.parametersHorizontal,
                data.parametersVertical,
                data.parametersEyes,
                data.returnSpeed,
                eyeState,
              );
            },
            data.throwDuration * 500,
            data.throwAngleMin,
            data.throwAngleMax,
          );
          ...
          if (impactDecal != null)
            setTimeout(
              function () {
                ...
                hit.style.left =
                  window.innerWidth * xPos -
                  (impact.width * impactDecal.scale * sizeScale) / 2 +
                  randH +
                  "px";
                hit.style.top =
                  window.innerHeight * yPos -
                  (impact.height * impactDecal.scale * sizeScale) / 2 +
                  randV +
                  "px";
                hit.src =
                  "decals/" +
                  encodeURIComponent(impactDecal.location.substr(7));
                hit.style.width =
                  impact.width * impactDecal.scale * sizeScale + "px";
                hit.style.height =
                  impact.height * impactDecal.scale * sizeScale + "px";
                document.querySelector("body").appendChild(hit);

                setTimeout(function () {
                  hit.remove();
                }, impactDecal.duration * 1000);
              },
              data.throwDuration * 500 + data.delay,
            );
          ...
          if (!data.physicsSim)
            setTimeout(
              ...
              data.throwDuration * 1000 + data.delay,
            );
          else {
            setTimeout(
              function () {
                ...
                var vX =
                  randV * 3 * (fromLeft ? -1 : 1) * data.physicsHorizontal;
                var vY =
                  (1 - randV) *
                  10 *
                  (angle < 0 ? -1 : 0.5) *
                  data.physicsVertical;
                ...
                if (data.physicsFPS != physicsFPS) {
                  ...
                  if (physicsSimulator) clearInterval(physicsSimulator);
                  physicsSimulator = setInterval(
                    simulatePhysics,
                    1000 / physicsFPS,
                  );
                }
...
var physicsFPS = 60,
  physicsGravityMult = 1,
  physicsGravityReverse = false;
var objects = [];
...
function simulatePhysics() {
  for (var i = 0; i < objects.length; i++) {
    ...
    objects[i].vY +=
      30 *
      physicsGravityMult *
      (physicsGravityReverse ? -1 : 1) *
      (1 / physicsFPS);
    objects[i].y += objects[i].vY;
    objects[i].element.style.transform =
      "translate(" + objects[i].x + "vw," + objects[i].y + "vh)";
...
var parametersH = ["FaceAngleX", "FaceAngleZ", "FacePositionX"],
  parametersV = ["FaceAngleY", "FacePositionY"],
  parametersE = ["EyeOpenLeft", "EyeOpenRight"];
var flinchWeight = 0,
  flinchInterval;
function flinch(
  multH,
  ...
  for (var i = 0; i < paramH.length; i++)
    parameterValues.push({
      id: paramH[i][0],
      value: (multH < 0 ? paramH[i][2] : paramH[i][3]) * mag,
    });
  for (var i = 0; i < paramV.length; i++)
    parameterValues.push({
      id: paramV[i][0],
      value:
        (((angle > 0 ? paramV[i][2] : paramV[i][3]) * Math.abs(angle)) / 45) *
        mag,
    });
...
function flinchReturn(
  ...
  for (var i = 0; i < paramH.length; i++)
    parameterValues.push({
      id: paramH[i][0],
      value: (multH < 0 ? paramH[i][2] : paramH[i][3]) * mag * flinchWeight,
    });
  for (var i = 0; i < paramV.length; i++)
    parameterValues.push({
      id: paramV[i][0],
      value:
        (((multH * angle > 0 ? paramV[i][2] : paramV[i][3]) * Math.abs(angle)) /
          45) *
        mag *
        flinchWeight,
    });
