...
const statusDesc = [
  ...
  [
    "<p>If this message doesn't disappear after a few seconds, please refresh the \"bonker.html\" Browser Source.</p><p>If that doesn't work, please ensure the VTube Studio API is enabled on port <mark>",
    "</mark>.</p><p>If you have a Custom VTube Studio IP set in the settings, please ensure it is correct.</p>",
  ],
  ...
  [
    "<p>The port <mark>",
    "</mark> is already in use. Another process may be using this port.</p><p>Try changing the Browser Source Port in Settings, under Advanced Settings.</p><p>It should be some number between 1024 and 65535.</p>",
  ],
...
document.querySelector("#itchLink a").addEventListener("click", () => {
...
async function setStatus(_, message) {
  ...
  document.querySelector("#headerStatusInner").innerHTML =
    statusTitle[status] + (status != 0 ? " (Click)" : "");
  ...
  if (status == 5)
    document.querySelector("#statusDesc").innerHTML =
      statusDesc[status][0] +
      (await getData("portVTubeStudio")) +
      statusDesc[status][1];
  else if (status == 9)
    document.querySelector("#statusDesc").innerHTML =
      statusDesc[status][0] +
      (await getData("portThrower")) +
      statusDesc[status][1];
  else document.querySelector("#statusDesc").innerHTML = statusDesc[status];

  if (status == 3 || status == 4 || status == 7) {
    if (status == 7)
      document
        .querySelector("#nextCalibrate")
        .querySelector(".innerTopButton").innerText = "Start Calibration";
    else if (status == 3)
      document
        .querySelector("#nextCalibrate")
        .querySelector(".innerTopButton").innerText = "Continue Calibration";
    else if (status == 4)
      document
        .querySelector("#nextCalibrate")
        .querySelector(".innerTopButton").innerText = "Confirm Calibration";
    document.querySelector("#calibrateButtons").classList.remove("hidden");
  } else document.querySelector("#calibrateButtons").classList.add("hidden");
}
...
document.querySelector("#newImage").addEventListener("click", () => {
  document.querySelector("#loadImage").click();
});
      ...
      while (
        fs.existsSync(
          userDataPath +
            ...
            append +
            imageFile.name.substr(imageFile.name.lastIndexOf(".")),
        )
      ) {
        ...
        if (target.equals(source)) append = append == "" ? 2 : append + 1;
        else break;
      }
    }
    var filename =
...
document
  ...
  .querySelector(".selectAll input")
  .addEventListener("change", async () => {
    document
      .querySelector("#imageTable")
      .querySelectorAll(".imageEnabled")
      .forEach((element) => {
        element.checked = document
          .querySelector("#imageTable")
          .querySelector(".selectAll input").checked;
      });
    var throws = await getData("throws");
    for (var i = 0; i < throws.length; i++)
      throws[i].enabled = document
        .querySelector("#imageTable")
        .querySelector(".selectAll input").checked;
    setData("throws", throws);
  });
...
async function openImages() {
  ...
  document
    .querySelector("#imageTable")
    .querySelector(".selectAll input").checked = allEnabled;

  if (throws == null) setData("throws", []);
  else {
    throws.forEach((_, index) => {
      if (fs.existsSync(userDataPath + "/" + throws[index].location)) {
        ...
        row.querySelector(".imageLabel").innerText = throws[
          index
        ].location.substr(throws[index].location.lastIndexOf("/") + 1);

        row.querySelector(".imageImage").src =
          userDataPath + "/" + throws[index].location;

        var pixel = throws[index].pixel != null ? throws[index].pixel : false;
        row.querySelector(".imageImage").style.imageRendering = pixel
          ? "pixelated"
          : "auto";

        row.querySelector(".imageEnabled").checked = throws[index].enabled;
        row
          .querySelector(".imageEnabled")
          .addEventListener("change", async () => {
            var throws = await getData("throws");
...
async function openImagesCustom(customName) {
  ...
  document.querySelector("#newImageCustom").addEventListener("click", () => {
    document.querySelector("#loadImageCustom").click();
  });
  document.querySelector("#loadImageCustom").addEventListener("change", () => {
    loadImageCustom(customName);
  });
  ...
  document
    .querySelector("#imageTableCustom")
    .querySelector(".selectAll input").checked = allEnabled;
      ...
      for (var i = 0; i < throws.length; i++) {
        if (
          document
            .querySelector("#imageTableCustom")
            .querySelector(".selectAll input").checked &&
          !throws[i].customs.includes(customName)
        )
  ...
  if (throws == null) setData("throws", []);
    ...
    throws.forEach((_, index) => {
      if (fs.existsSync(userDataPath + "/" + throws[index].location)) {
        ...
        row.querySelector(".imageEnabled").checked =
          throws[index].customs.includes(customName);
        row
          .querySelector(".imageEnabled")
          .addEventListener("change", async () => {
            var throws = await getData("throws");
            if (
              ...
              throws[index].customs.splice(
                throws[index].customs.indexOf(customName),
                1,
              );
            else if (
              row.querySelector(".imageEnabled").checked &&
    ...
    if (soundFile.path != userDataPath + "\\impacts\\" + soundFile.name) {
      while (
        fs.existsSync(
          userDataPath +
            "/impacts/" +
            soundFile.name.substr(0, soundFile.name.lastIndexOf(".")) +
            append +
            soundFile.name.substr(soundFile.name.lastIndexOf(".")),
        )
      ) {
        var target = fs.readFileSync(
          userDataPath +
...
async function openSoundsCustom(customName) {
  ...
  document.querySelector("#newSoundCustom").addEventListener("click", () => {
    document.querySelector("#loadSoundCustom").click();
  });
  document.querySelector("#loadSoundCustom").addEventListener("change", () => {
    loadSoundCustom(customName);
  });
  ...
  document
    .querySelector("#soundTableCustom")
    .querySelector(".selectAll input").checked = allEnabled;
      ...
      for (var i = 0; i < impacts.length; i++) {
        if (
          document
            .querySelector("#soundTableCustom")
            .querySelector(".selectAll input").checked &&
          !impacts[i].customs.includes(customName)
        )
  ...
  document
    .querySelector("#soundTableCustom")
    .querySelectorAll(".soundRow")
    .forEach((element) => {
      element.remove();
    });

  if (impacts == null) setData("impacts", []);
  else {
    impacts.forEach((_, index) => {
      if (fs.existsSync(userDataPath + "/" + impacts[index].location)) {
        ...
        row.removeAttribute("hidden");
        row.querySelector(".imageLabel").innerText = impacts[
          index
        ].location.substr(impacts[index].location.lastIndexOf("/") + 1);
        document.querySelector("#soundTableCustom").appendChild(row);

        row.querySelector(".imageEnabled").checked =
          impacts[index].customs.includes(customName);
        row
          .querySelector(".imageEnabled")
          .addEventListener("change", async () => {
            var impacts = await getData("impacts");
            if (
              ...
              impacts[index].customs.splice(
                impacts[index].customs.indexOf(customName),
                1,
              );
            else if (
              row.querySelector(".imageEnabled").checked &&
...
async function openImpactDecals(customName) {
  ...
  document.querySelector("#newImpactDecal").addEventListener("click", () => {
    document.querySelector("#loadImpactDecal").click();
  });
  document.querySelector("#loadImpactDecal").addEventListener("change", () => {
    loadImpactDecal(customName);
  });
  ...
  document
    .querySelector("#impactDecalsTable")
    .querySelector(".selectAll input").checked = allEnabled;
  ...
  document
    ...
    .addEventListener("change", async () => {
      ...
      for (var i = 0; i < customBonks[customName].impactDecals.length; i++)
        customBonks[customName].impactDecals[i].enabled = document
          .querySelector("#impactDecalsTable")
          .querySelector(".selectAll input").checked;
      setData("customBonks", customBonks);
    });
  ...
  customBonks[customName].impactDecals.forEach((_, index) => {
    if (
      fs.existsSync(
        userDataPath +
          "/" +
          customBonks[customName].impactDecals[index].location,
      )
    ) {
      ...
      row.classList.add("imageRow");
      row.removeAttribute("hidden");
      row.querySelector(".imageLabel").innerText = customBonks[
        customName
      ].impactDecals[index].location.substr(
        customBonks[customName].impactDecals[index].location.lastIndexOf("/") +
          1,
      );
      ...
      row.querySelector(".imageEnabled").checked =
        customBonks[customName].impactDecals[index].enabled;
      row
        .querySelector(".imageEnabled")
        .addEventListener("change", async () => {
          var customBonks = await getData("customBonks");
          ...
          for (
            var i = 0;
            i < customBonks[customName].impactDecals.length;
            i++
          ) {
            if (!customBonks[customName].impactDecals[i].enabled) {
              allEnabled = false;
      ...
      row.querySelector(".decalDuration").value =
        customBonks[customName].impactDecals[index].duration;
      row
        .querySelector(".decalDuration")
        .addEventListener("change", async () => {
          var customBonks = await getData("customBonks");
          clampValue(row.querySelector(".decalDuration").value, 0, null);
          customBonks[customName].impactDecals[index].duration = parseFloat(
            row.querySelector(".decalDuration").value,
          );
          setData("customBonks", customBonks);
        });

      row.querySelector(".decalScale").value =
        customBonks[customName].impactDecals[index].scale;
      row.querySelector(".decalScale").addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        clampValue(row.querySelector(".decalScale"), 0, null);
        customBonks[customName].impactDecals[index].scale = parseFloat(
          row.querySelector(".decalScale").value,
        );
        setData("customBonks", customBonks);
      });
...
async function openWindupSounds(customName) {
  ...
  document.querySelector("#newWindupSound").addEventListener("click", () => {
    document.querySelector("#loadWindupSound").click();
  });
  document.querySelector("#loadWindupSound").addEventListener("change", () => {
    loadWindupSound(customName);
  });
  ...
  document
    .querySelector("#windupSoundTable")
    .querySelector(".selectAll input").checked = allEnabled;
  ...
  document
    ...
    .addEventListener("change", async () => {
      ...
      for (var i = 0; i < customBonks[customName].windupSounds.length; i++)
        customBonks[customName].windupSounds[i].enabled = document
          .querySelector("#windupSoundTable")
          .querySelector(".selectAll input").checked;
      setData("customBonks", customBonks);
    });
  ...
  customBonks[customName].windupSounds.forEach((_, index) => {
    if (
      fs.existsSync(
        userDataPath +
          "/" +
          customBonks[customName].windupSounds[index].location,
      )
    ) {
      ...
      row.querySelector(".imageLabel").innerText = customBonks[
        customName
      ].windupSounds[index].location.substr(
        customBonks[customName].windupSounds[index].location.lastIndexOf("/") +
          1,
      );
      ...
      row.querySelector(".imageEnabled").checked =
        customBonks[customName].windupSounds[index].enabled;
      row
        .querySelector(".imageEnabled")
        .addEventListener("change", async () => {
          var customBonks = await getData("customBonks");
          ...
          for (
            var i = 0;
            i < customBonks[customName].windupSounds.length;
            i++
          ) {
            if (!customBonks[customName].windupSounds[i].enabled) {
              allEnabled = false;
      ...
      row.querySelector(".soundVolume").value =
        customBonks[customName].windupSounds[index].volume;
      row.querySelector(".soundVolume").addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        clampValue(row.querySelector(".soundVolume"), 0, 1);
        customBonks[customName].windupSounds[index].volume = parseFloat(
          row.querySelector(".soundVolume").value,
        );
        setData("customBonks", customBonks);
      });
...
document
  .querySelector("#bit1")
  .querySelector(".bitImageScale")
  .addEventListener("change", async () => {
    var bitThrows = await getData("bitThrows");
    bitThrows.one.scale = parseFloat(
      document.querySelector("#bit1").querySelector(".bitImageScale").value,
    );
    setData("bitThrows", bitThrows);
  });
document.querySelector("#bitImageAdd1").addEventListener("click", () => {
  document.querySelector("#loadBitImageone").click();
});
document.querySelector("#loadBitImageone").addEventListener("change", () => {
  loadBitImage("one");
});

document
  .querySelector("#bit100")
  .querySelector(".bitImageScale")
  .addEventListener("change", async () => {
    var bitThrows = await getData("bitThrows");
    bitThrows.oneHundred.scale = parseFloat(
      document.querySelector("#bit100").querySelector(".bitImageScale").value,
    );
    setData("bitThrows", bitThrows);
  });
document.querySelector("#bitImageAdd100").addEventListener("click", () => {
  document.querySelector("#loadBitImageoneHundred").click();
});
document
  .querySelector("#loadBitImageoneHundred")
  .addEventListener("change", () => {
    loadBitImage("oneHundred");
  });

document
  .querySelector("#bit1000")
  .querySelector(".bitImageScale")
  .addEventListener("change", async () => {
    var bitThrows = await getData("bitThrows");
    bitThrows.oneThousand.scale = parseFloat(
      document.querySelector("#bit1000").querySelector(".bitImageScale").value,
    );
    setData("bitThrows", bitThrows);
  });
document.querySelector("#bitImageAdd1000").addEventListener("click", () => {
  document.querySelector("#loadBitImageoneThousand").click();
});
document
  .querySelector("#loadBitImageoneThousand")
  .addEventListener("change", () => {
    loadBitImage("oneThousand");
  });

document
  .querySelector("#bit5000")
  .querySelector(".bitImageScale")
  .addEventListener("change", async () => {
    var bitThrows = await getData("bitThrows");
    bitThrows.fiveThousand.scale = parseFloat(
      document.querySelector("#bit5000").querySelector(".bitImageScale").value,
    );
    setData("bitThrows", bitThrows);
  });
document.querySelector("#bitImageAdd5000").addEventListener("click", () => {
  document.querySelector("#loadBitImagefiveThousand").click();
});
document
  .querySelector("#loadBitImagefiveThousand")
  .addEventListener("change", () => {
    loadBitImage("fiveThousand");
  });

document
  .querySelector("#bit10000")
  .querySelector(".bitImageScale")
  .addEventListener("change", async () => {
    var bitThrows = await getData("bitThrows");
    bitThrows.tenThousand.scale = parseFloat(
      document.querySelector("#bit10000").querySelector(".bitImageScale").value,
    );
    setData("bitThrows", bitThrows);
  });
document.querySelector("#bitImageAdd10000").addEventListener("click", () => {
  document.querySelector("#loadBitImagetenThousand").click();
});
document
  .querySelector("#loadBitImagetenThousand")
  .addEventListener("change", () => {
    loadBitImage("tenThousand");
  });
...
async function openBitImages() {
  ...
  document.querySelector("#bit1").querySelector(".imageImage").src =
    userDataPath + "/" + bitThrows.one.location;
  document.querySelector("#bit1").querySelector(".bitImageScale").value =
    bitThrows.one.scale;

  document.querySelector("#bit100").querySelector(".imageImage").src =
    userDataPath + "/" + bitThrows.oneHundred.location;
  document.querySelector("#bit100").querySelector(".bitImageScale").value =
    bitThrows.oneHundred.scale;

  document.querySelector("#bit1000").querySelector(".imageImage").src =
    userDataPath + "/" + bitThrows.oneThousand.location;
  document.querySelector("#bit1000").querySelector(".bitImageScale").value =
    bitThrows.oneThousand.scale;

  document.querySelector("#bit5000").querySelector(".imageImage").src =
    userDataPath + "/" + bitThrows.fiveThousand.location;
  document.querySelector("#bit5000").querySelector(".bitImageScale").value =
    bitThrows.fiveThousand.scale;

  document.querySelector("#bit10000").querySelector(".imageImage").src =
    userDataPath + "/" + bitThrows.tenThousand.location;
  document.querySelector("#bit10000").querySelector(".bitImageScale").value =
    bitThrows.tenThousand.scale;
}

document
  .querySelector("#loadImageSound")
  .addEventListener("change", loadImageSound);
...
async function openImageDetails() {
  ...
  document
    .querySelector("#testImage")
    .addEventListener("click", () => testItem(currentImageIndex));

  const details = document.querySelector("#imageDetails");

  details.querySelector(".imageLabel").innerText = throws[
    currentImageIndex
  ].location.substr(throws[currentImageIndex].location.lastIndexOf("/") + 1);

  details.querySelector(".imageImage").src =
    userDataPath + "/" + throws[currentImageIndex].location;
  details.querySelector(".imageImage").style.transform =
    "scale(" + throws[currentImageIndex].scale + ")";
  details.querySelector(".imageWeight").value =
    throws[currentImageIndex].weight;
  ...
  details.querySelector(".imagePixel").checked =
    throws[currentImageIndex].pixel;
  details.querySelector(".imageImage").style.imageRendering = throws[
    currentImageIndex
  ].pixel
    ? "pixelated"
    : "auto";
  ...
  if (throws[currentImageIndex].sound != null) {
    details.querySelector(".imageSoundName").value =
      throws[currentImageIndex].sound.substr(8);
  ...
  details.querySelector(".imageWeight").addEventListener("change", async () => {
    var throws = await getData("throws");
    throws[currentImageIndex].weight = parseFloat(
      details.querySelector(".imageWeight").value,
    );
    setData("throws", throws);
  });
  ...
  details.querySelector(".imageScale").addEventListener("change", async () => {
    var throws = await getData("throws");
    throws[currentImageIndex].scale = parseFloat(
      details.querySelector(".imageScale").value,
    );
    details.querySelector(".imageImage").style.transform =
      "scale(" + throws[currentImageIndex].scale + ")";
    setData("throws", throws);
  ...
  details.querySelector(".imageSoundVolume").value =
    throws[currentImageIndex].volume;
  details
    .querySelector(".imageSoundVolume")
    .addEventListener("change", async () => {
      var throws = await getData("throws");
      throws[currentImageIndex].volume = parseFloat(
        details.querySelector(".imageSoundVolume").value,
      );
      setData("throws", throws);
    });
  ...
  details
    .querySelector(".imageSoundRemove")
    .addEventListener("click", async () => {
      var throws = await getData("throws");
...
document.querySelector("#newSound").addEventListener("click", () => {
  document.querySelector("#loadSound").click();
});
  ...
  if (impacts == null) setData("impacts", []);
    ...
    impacts.forEach((_, index) => {
      if (fs.existsSync(userDataPath + "/" + impacts[index].location)) {
        ...
        row
          .querySelector(".soundVolume")
          .addEventListener("change", async () => {
            var impacts = await getData("impacts");
            clampValue(row.querySelector(".soundVolume"), 0, 1);
            impacts[index].volume = parseFloat(
              row.querySelector(".soundVolume").value,
            );
            setData("impacts", impacts);
          });
...
document.querySelector("#newBitSound").addEventListener("click", () => {
  document.querySelector("#loadBitSound").click();
});
document
  .querySelector("#loadBitSound")
  .addEventListener("change", loadBitSound);
...
async function openBitSounds() {
  ...
  document.querySelectorAll(".bitSoundRow").forEach((element) => {
    element.remove();
  });
...
async function addBonk() {
  var newBonkNumber = 1;
  var customBonks = await getData("customBonks");
  if (customBonks == null) customBonks = {};

  while (customBonks["Custom Bonk " + newBonkNumber] != null) newBonkNumber++;
...
async function bonkDetails(customBonkName) {
  ...
  if (customBonks[customBonkName] != null) {
    ...
    bonkDetailsTable
      .querySelector(".bonkName")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        if (
          customBonks[bonkDetailsTable.querySelector(".bonkName").value] == null
        ) {
          customBonks[bonkDetailsTable.querySelector(".bonkName").value] =
            customBonks[customBonkName];
          delete customBonks[customBonkName];

          var throws = await getData("throws");
          for (var i = 0; i < throws.length; i++) {
            if (throws[i].customs.includes(customBonkName)) {
              throws[i].customs.splice(
                throws[i].customs.indexOf(customBonkName),
                1,
              );
              throws[i].customs.push(
                bonkDetailsTable.querySelector(".bonkName").value,
              );
            }
          }
          setData("throws", throws);
          ...
          for (var i = 0; i < impacts.length; i++) {
            if (impacts[i].customs.includes(customBonkName)) {
              impacts[i].customs.splice(
                impacts[i].customs.indexOf(customBonkName),
                1,
              );
              impacts[i].customs.push(
                bonkDetailsTable.querySelector(".bonkName").value,
              );
            }
          }
    ...
    // Barrage Count
    bonkDetailsTable.querySelector(".barrageCount").value =
      customBonks[customBonkName].barrageCount;
    bonkDetailsTable
      .querySelector(".barrageCount")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].barrageCount = parseInt(
          bonkDetailsTable.querySelector(".barrageCount").value,
        );
        setData("customBonks", customBonks);
      });
    ...
    // Barrage Frequency
    bonkDetailsTable.querySelector(".barrageFrequencyOverride").checked =
      customBonks[customBonkName].barrageFrequencyOverride;
    bonkDetailsTable.querySelector(".barrageFrequency").disabled =
      !customBonks[customBonkName].barrageFrequencyOverride;
    bonkDetailsTable
      .querySelector(".barrageFrequencyOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].barrageFrequencyOverride =
          bonkDetailsTable.querySelector(".barrageFrequencyOverride").checked;
    ...
    bonkDetailsTable.querySelector(".barrageFrequency").value =
      customBonks[customBonkName].barrageFrequency;
    bonkDetailsTable
      .querySelector(".barrageFrequency")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].barrageFrequency = parseFloat(
          bonkDetailsTable.querySelector(".barrageFrequency").value,
        );
        setData("customBonks", customBonks);
      });
    ...
    // Throw Duration
    bonkDetailsTable.querySelector(".throwDurationOverride").checked =
      customBonks[customBonkName].throwDurationOverride;
    bonkDetailsTable.querySelector(".throwDuration").disabled =
      !customBonks[customBonkName].throwDurationOverride;
    bonkDetailsTable
      .querySelector(".throwDurationOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwDurationOverride =
          bonkDetailsTable.querySelector(".throwDurationOverride").checked;
    ...
    bonkDetailsTable.querySelector(".throwDuration").value =
      customBonks[customBonkName].throwDuration;
    bonkDetailsTable
      .querySelector(".throwDuration")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwDuration = parseFloat(
          bonkDetailsTable.querySelector(".throwDuration").value,
        );
        setData("customBonks", customBonks);
      });
    ...
    // Spin Speed
    bonkDetailsTable.querySelector(".spinSpeedOverride").checked =
      customBonks[customBonkName].spinSpeedOverride;
    bonkDetailsTable.querySelector(".spinSpeedMin").disabled =
      !customBonks[customBonkName].spinSpeedOverride;
    bonkDetailsTable.querySelector(".spinSpeedMax").disabled =
      !customBonks[customBonkName].spinSpeedOverride;
    bonkDetailsTable
      .querySelector(".spinSpeedOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].spinSpeedOverride =
          bonkDetailsTable.querySelector(".spinSpeedOverride").checked;
    ...
    bonkDetailsTable.querySelector(".spinSpeedMin").value =
      customBonks[customBonkName].spinSpeedMin;
    bonkDetailsTable
      .querySelector(".spinSpeedMin")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customName].spinSpeedMin = parseFloat(
          bonkDetailsTable.querySelector(".spinSpeedMin").value,
        );
        setData("customBonks", customBonks);
      });
    ...
    // Throw Angle Max
    bonkDetailsTable.querySelector(".spinSpeedMax").value =
      customBonks[customBonkName].spinSpeedMax;
    bonkDetailsTable
      .querySelector(".spinSpeedMax")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].spinSpeedMax = parseFloat(
          bonkDetailsTable.querySelector(".spinSpeedMax").value,
        );
        setData("customBonks", customBonks);
      });
    ...
    // Throw Angle
    bonkDetailsTable.querySelector(".throwAngleOverride").checked =
      customBonks[customBonkName].throwAngleOverride;
    bonkDetailsTable.querySelector(".throwAngleMin").disabled =
      !customBonks[customBonkName].throwAngleOverride;
    bonkDetailsTable.querySelector(".throwAngleMax").disabled =
      !customBonks[customBonkName].throwAngleOverride;
    bonkDetailsTable
      .querySelector(".throwAngleOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwAngleOverride =
          bonkDetailsTable.querySelector(".throwAngleOverride").checked;
    ...
    bonkDetailsTable.querySelector(".throwAngleMin").value =
      customBonks[customBonkName].throwAngleMin;
    bonkDetailsTable
      .querySelector(".throwAngleMin")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwAngleMin = parseInt(
          bonkDetailsTable.querySelector(".throwAngleMin").value,
        );
        setData("customBonks", customBonks);
      });

    bonkDetailsTable.querySelector(".throwAngleMax").value =
      customBonks[customBonkName].throwAngleMax;
    bonkDetailsTable
      .querySelector(".throwAngleMax")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwAngleMax = parseInt(
          bonkDetailsTable.querySelector(".throwAngleMax").value,
        );
        setData("customBonks", customBonks);
      });
    ...
    // Throw Direction
    bonkDetailsTable.querySelector(".throwDirectionOverride").checked =
      customBonks[customBonkName].throwDirectionOverride;
    bonkDetailsTable.querySelector(".throwDirection").disabled =
      !customBonks[customBonkName].throwDirectionOverride;
    bonkDetailsTable
      .querySelector(".throwDirectionOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwDirectionOverride =
          bonkDetailsTable.querySelector(".throwDirectionOverride").checked;
    ...
    bonkDetailsTable.querySelector(".throwDirection").value =
      customBonks[customBonkName].throwDirection;
    bonkDetailsTable
      .querySelector(".throwDirection")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].throwDirection =
          bonkDetailsTable.querySelector(".throwDirection").value;
    ...
    // Items
    bonkDetailsTable.querySelector(".imagesOverride").checked =
      customBonks[customBonkName].itemsOverride;
    bonkDetailsTable.querySelector(".images").disabled =
      !customBonks[customBonkName].itemsOverride;
    bonkDetailsTable
      .querySelector(".imagesOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].itemsOverride =
          bonkDetailsTable.querySelector(".imagesOverride").checked;
    ...
    // Sounds
    bonkDetailsTable.querySelector(".soundsOverride").checked =
      customBonks[customBonkName].soundsOverride;
    bonkDetailsTable.querySelector(".sounds").disabled =
      !customBonks[customBonkName].soundsOverride;
    bonkDetailsTable
      .querySelector(".soundsOverride")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].soundsOverride =
          bonkDetailsTable.querySelector(".soundsOverride").checked;
    ...
    // Windup Delay
    bonkDetailsTable.querySelector(".windupDelay").value =
      customBonks[customBonkName].windupDelay;
    bonkDetailsTable
      .querySelector(".windupDelay")
      .addEventListener("change", async () => {
        var customBonks = await getData("customBonks");
        customBonks[customBonkName].windupDelay = parseFloat(
          bonkDetailsTable.querySelector(".windupDelay").value,
        );
        setData("customBonks", customBonks);
      });
...
async function openBonks() {
  ...
  document.querySelectorAll(".customBonkRow").forEach((element) => {
    element.remove();
  });

  if (customBonks == null) setData("customBonks", {});
  else {
    for (const key in customBonks) {
      ...
      row.querySelector(".imageRemove").addEventListener("click", async () => {
        ...
        for (var i = 0; i < eventType.length; i++) {
          if (eventType[i].bonkType == key) eventType[i].bonkType = "single";
        }
        ...
        if (eventType == key) setData("subType", "barrage");

        eventType = await getData("subGiftType");
        if (eventType == key) setData("subGiftType", "barrage");

        eventType = await getData("charityType");
        if (eventType == key) setData("charityType", "barrage");
...
async function openTestBonks() {
  ...
  document.querySelectorAll(".testCustom").forEach((element) => {
    element.remove();
  });
...
async function openEvents() {
  ...
  document.querySelectorAll(".redeemsRow").forEach((element) => {
    element.remove();
  });
  ...
  redeems.forEach((_, index) => {
    ...
    row.querySelector(".redeemName").innerHTML =
      redeems[index].name == null
        ? '<b class="errorText">Unassigned</b>'
        : redeems[index].name;
  ...
  document.querySelectorAll(".commandsRow").forEach((element) => {
    element.remove();
  });
  ...
  while (node.childElementCount > 4) node.removeChild(node.lastChild);
...
ipcRenderer.on("doneWriting", () => {
  if (--isWriting < 0) isWriting = 0;
});

// Get requested data, waiting for any current writes to finish first
async function getData(field) {
  while (isWriting > 0) await new Promise((resolve) => setTimeout(resolve, 10));
  ...
  if (typeof field === "string" || field instanceof String) return field.trim();
  return field;
}
...
async function setPorts() {
  fs.writeFileSync(
    ...
    "const ports = [ " +
      ...
      (await getData("portVTubeStudio")) +
      ' ]; const ips = [ "' +
      (await getData("ipThrower")) +
      '", "' +
      (await getData("ipVTubeStudio")) +
      '" ];',
  );
}
...
async function loadData(field) {
  ...
  if (thisData != null) {
    ...
    if (field == "udpPort") ipcRenderer.send("changeUdpPort", parseInt(val));
  }
}
...
function copyFilesToDirectory() {
  folders.forEach((folder) => {
    ...
    fs.readdirSync(userDataPath + "/" + folder).forEach((file) => {
      fs.copyFileSync(
        userDataPath + "/" + folder + "/" + file,
        __dirname + "/" + folder + "/" + file,
      );
    });
  });
}
...
window.onload = async function () {
  ...
  if (
    !fs.existsSync(userDataPath + "/data.json") &&
    fs.existsSync(__dirname + "/data.json")
  )
  ...
  if (tray == null) setData("minimizeToTray", false);

  // UPDATE 1.22
  // Changed "Return Speed" to "Return Time"
  var prevVer = await getData("version");
  if (prevVer != null && prevVer < 1.22) setData("returnSpeed", 0.3);

  var commands = await getData("commands");
  if (commands != null) {
    for (const key in commands) {
      if (commands[key].modOnly == null) commands[key].modOnly = false;
    }
  ...
  var prevVer = await getData("version");
  if (prevVer == null || prevVer < 1.22) setData("throwDirection", "weighted");
  ...
  if (ipThrower == null) setData("ipThrower", "");

  var ipVTubeStudio = await getData("ipVTubeStudio");
  if (ipVTubeStudio == null) setData("ipVTubeStudio", "");
...
document
  .querySelector("#followEnabled")
  .addEventListener("change", () =>
    setData("followEnabled", document.querySelector("#followEnabled").checked),
  );
document
  .querySelector("#subEnabled")
  .addEventListener("change", () =>
    setData("subEnabled", document.querySelector("#subEnabled").checked),
  );
document
  .querySelector("#subGiftEnabled")
  .addEventListener("change", () =>
    setData(
      "subGiftEnabled",
      document.querySelector("#subGiftEnabled").checked,
    ),
  );
document
  .querySelector("#charityEnabled")
  .addEventListener("change", () =>
    setData(
      "charityEnabled",
      document.querySelector("#charityEnabled").checked,
    ),
  );
document
  .querySelector("#bitsEnabled")
  .addEventListener("change", () =>
    setData("bitsEnabled", document.querySelector("#bitsEnabled").checked),
  );
document
  .querySelector("#raidEnabled")
  .addEventListener("change", () =>
    setData("raidEnabled", document.querySelector("#raidEnabled").checked),
  );

document
  .querySelector("#followType")
  .addEventListener("change", () =>
    setData("followType", document.querySelector("#followType").value),
  );
document
  .querySelector("#subType")
  .addEventListener("change", () =>
    setData("subType", document.querySelector("#subType").value),
  );
document
  .querySelector("#subGiftType")
  .addEventListener("change", () =>
    setData("subGiftType", document.querySelector("#subGiftType").value),
  );
document
  .querySelector("#charityType")
  .addEventListener("change", () =>
    setData("charityType", document.querySelector("#charityType").value),
  );
document.querySelector("#bitsMinDonation").addEventListener("change", () => {
  clampValue(document.querySelector("#bitsMinDonation"), 0, null);
  setData(
    "bitsMinDonation",
    parseInt(document.querySelector("#bitsMinDonation").value),
  );
});
document
  .querySelector("#bitsMaxBarrageCount")
  .addEventListener("change", () => {
    clampValue(document.querySelector("#bitsMaxBarrageCount"), 0, null);
    setData(
      "bitsMaxBarrageCount",
      parseInt(document.querySelector("#bitsMaxBarrageCount").value),
    );
  });

document.querySelector("#raidMinRaiders").addEventListener("change", () => {
  setData(
    "raidMinRaiders",
    parseInt(document.querySelector("#raidMinRaiders").value),
  );
});
document
  .querySelector("#raidMinBarrageCount")
  .addEventListener("change", () => {
    clampValue(
      document.querySelector("#raidMinBarrageCount"),
      0,
      parseFloat(document.querySelector("#raidMaxBarrageCount").value),
    );
    setData(
      "raidMinBarrageCount",
      parseInt(document.querySelector("#raidMinBarrageCount").value),
    );
  });
document
  .querySelector("#raidMaxBarrageCount")
  .addEventListener("change", () => {
    clampValue(
      document.querySelector("#raidMaxBarrageCount"),
      parseFloat(document.querySelector("#raidMinBarrageCount").value),
      null,
    );
    setData(
      "raidMaxBarrageCount",
      parseInt(document.querySelector("#raidMaxBarrageCount").value),
    );
  });

document.querySelector("#followCooldown").addEventListener("change", () => {
  clampValue(document.querySelector("#followCooldown"), 0, null);
  setData(
    "followCooldown",
    parseFloat(document.querySelector("#followCooldown").value),
  );
});
document.querySelector("#subCooldown").addEventListener("change", () => {
  clampValue(document.querySelector("#subCooldown"), 0, null);
  setData(
    "subCooldown",
    parseFloat(document.querySelector("#subCooldown").value),
  );
});
document.querySelector("#subGiftCooldown").addEventListener("change", () => {
  clampValue(document.querySelector("#subGiftCooldown"), 0, null);
  setData(
    "subGiftCooldown",
    parseFloat(document.querySelector("#subGiftCooldown").value),
  );
});
document.querySelector("#charityCooldown").addEventListener("change", () => {
  clampValue(document.querySelector("#charityCooldown"), 0, null);
  setData(
    "charityCooldown",
    parseFloat(document.querySelector("#charityCooldown").value),
  );
});
document.querySelector("#bitsCooldown").addEventListener("change", () => {
  clampValue(document.querySelector("#bitsCooldown"), 0, null);
  setData(
    "bitsCooldown",
    parseFloat(document.querySelector("#bitsCooldown").value),
  );
});
document.querySelector("#raidCooldown").addEventListener("change", () => {
  clampValue(document.querySelector("#raidCooldown"), 0, null);
  setData(
    "raidCooldown",
    parseFloat(document.querySelector("#raidCooldown").value),
  );
});
document
  .querySelector("#raidEnabled")
...
document
  .querySelector("#bitsOnlySingle")
  .addEventListener("change", () =>
    setData(
      "bitsOnlySingle",
      document.querySelector("#bitsOnlySingle").checked,
    ),
  );
document
  .querySelector("#raidEmotes")
  .addEventListener("change", () =>
    setData("raidEmotes", document.querySelector("#raidEmotes").checked),
  );

document.querySelector("#barrageCount").addEventListener("change", () => {
  clampValue(document.querySelector("#barrageCount"), 0, null);
  setData(
    "barrageCount",
    parseInt(document.querySelector("#barrageCount").value),
  );
});
document.querySelector("#barrageFrequency").addEventListener("change", () => {
  clampValue(document.querySelector("#barrageFrequency"), 0, null);
  setData(
    "barrageFrequency",
    parseFloat(document.querySelector("#barrageFrequency").value),
  );
});
document.querySelector("#throwDuration").addEventListener("change", () => {
  clampValue(document.querySelector("#throwDuration"), 0.1, null);
  setData(
    "throwDuration",
    parseFloat(document.querySelector("#throwDuration").value),
  );
});
document.querySelector("#returnSpeed").addEventListener("change", () => {
  clampValue(document.querySelector("#returnSpeed"), 0, null);
  setData(
    "returnSpeed",
    parseFloat(document.querySelector("#returnSpeed").value),
  );
});
document.querySelector("#throwAngleMin").addEventListener("change", () => {
  clampValue(
    document.querySelector("#throwAngleMin"),
    -90,
    parseFloat(document.querySelector("#throwAngleMax").value),
  );
  setData(
    "throwAngleMin",
    parseFloat(document.querySelector("#throwAngleMin").value),
  );
});
document.querySelector("#throwAngleMax").addEventListener("change", () => {
  clampValue(
    document.querySelector("#throwAngleMax"),
    parseFloat(document.querySelector("#throwAngleMin").value),
    null,
  );
  setData(
...
document.querySelector("#throwDirection").addEventListener("change", () => {
  setData("throwDirection", document.querySelector("#throwDirection").value);
});
document.querySelector("#spinSpeedMin").addEventListener("change", () => {
  clampValue(
    document.querySelector("#spinSpeedMin"),
    0,
    parseFloat(document.querySelector("#spinSpeedMax").value),
  );
  setData(
    "spinSpeedMin",
    parseFloat(document.querySelector("#spinSpeedMin").value),
  );
});
document.querySelector("#spinSpeedMax").addEventListener("change", () => {
  clampValue(
    document.querySelector("#spinSpeedMax"),
    parseFloat(document.querySelector("#spinSpeedMin").value),
    null,
  );
  setData(
...
document
  .querySelector("#physicsSim")
  .addEventListener("change", () =>
    setData("physicsSim", document.querySelector("#physicsSim").checked),
  );
document.querySelector("#physicsFPS").addEventListener("change", () => {
  clampValue(document.querySelector("#physicsFPS"), 1, 60);
  setData(
    "physicsFPS",
    parseFloat(document.querySelector("#physicsFPS").value),
  );
});
document.querySelector("#physicsGravity").addEventListener("change", () => {
  clampValue(document.querySelector("#physicsGravity"), 0.01, null);
  setData(
    "physicsGravity",
    parseFloat(document.querySelector("#physicsGravity").value),
  );
});
document
  .querySelector("#physicsReverse")
  .addEventListener("change", () =>
    setData(
      "physicsReverse",
      document.querySelector("#physicsReverse").checked,
    ),
  );
document
  .querySelector("#physicsRotate")
  .addEventListener("change", () =>
    setData("physicsRotate", document.querySelector("#physicsRotate").checked),
  );
document.querySelector("#physicsHorizontal").addEventListener("change", () => {
  setData(
    "physicsHorizontal",
    parseFloat(document.querySelector("#physicsHorizontal").value),
  );
});
document.querySelector("#physicsVertical").addEventListener("change", () => {
  setData(
    "physicsVertical",
    parseFloat(document.querySelector("#physicsVertical").value),
  );
});
...
document.querySelector("#itemScaleMin").addEventListener("change", () => {
  clampValue(
    document.querySelector("#itemScaleMin"),
    0,
    parseFloat(document.querySelector("#itemScaleMax").value),
  );
  setData(
    "itemScaleMin",
    parseFloat(document.querySelector("#itemScaleMin").value),
  );
});
document.querySelector("#itemScaleMax").addEventListener("change", () => {
  clampValue(
    document.querySelector("#itemScaleMax"),
    parseFloat(document.querySelector("#itemScaleMin").value),
    null,
  );
  setData(
...
document.querySelector("#delay").addEventListener("change", () => {
  clampValue(document.querySelector("#delay"), 0, null);
  setData("delay", parseInt(document.querySelector("#delay").value));
});
document.querySelector("#volume").addEventListener("change", () => {
  clampValue(document.querySelector("#volume"), 0, 1);
  setData("volume", parseFloat(document.querySelector("#volume").value));
});
document.querySelector("#portThrower").addEventListener("change", () => {
  differentValue(
    document.querySelector("#portThrower"),
    document.querySelector("#portVTubeStudio"),
  );
  clampValue(document.querySelector("#portThrower"), 1024, 65535);
  setData(
    "portThrower",
    parseInt(document.querySelector("#portThrower").value),
  );
});
document.querySelector("#portVTubeStudio").addEventListener("change", () => {
  differentValue(
    document.querySelector("#portVTubeStudio"),
    document.querySelector("#portThrower"),
  );
  clampValue(document.querySelector("#portVTubeStudio"), 1024, 65535);
  setData(
    "portVTubeStudio",
    parseInt(document.querySelector("#portVTubeStudio").value),
  );
});

// INI LOGIKA UNTUK MENYIMPAN PORT KITA SAAT DIUBAH!
document.querySelector("#udpPort").addEventListener("change", () => {
  clampValue(document.querySelector("#udpPort"), 1024, 65535);
  let newPort = parseInt(document.querySelector("#udpPort").value);
  setData("udpPort", newPort);
  ipcRenderer.send("changeUdpPort", newPort);
});

document
  .querySelector("#ipThrower")
  .addEventListener("change", () =>
    setData("ipThrower", document.querySelector("#ipThrower").value),
  );
document
  .querySelector("#ipVTubeStudio")
  .addEventListener("change", () =>
    setData("ipVTubeStudio", document.querySelector("#ipVTubeStudio").value),
  );
document
  .querySelector("#minimizeToTray")
  .addEventListener("change", () =>
    setData(
      "minimizeToTray",
      document.querySelector("#minimizeToTray").checked,
    ),
  );

function differentValue(node, otherNode) {
  if (node.value == otherNode.value) node.value++;
}
...
var currentPanel = document.querySelector("#bonkImages"),
  playing = false;

// Window Event Listeners
document.querySelector("#header").addEventListener("click", () => {
  showPanelLarge("statusWindow");
});
...
document.querySelector("#calibrateButton").addEventListener("click", () => {
  showPanelLarge("statusWindow", true);
});
document.querySelector("#settingsButton").addEventListener("click", () => {
  showPanelLarge("settings");
});
document.querySelector("#testBonksButton").addEventListener("click", () => {
  showPanelLarge("testBonks");
});

document.querySelector("#imagesButton").addEventListener("click", () => {
  showPanel("bonkImages");
});
document.querySelector("#soundsButton").addEventListener("click", () => {
  showPanel("bonkSounds");
});
document.querySelector("#customButton").addEventListener("click", () => {
  showPanel("customBonks");
});
document.querySelector("#eventsButton").addEventListener("click", () => {
  showPanel("events");
});

document.querySelector("#imagesDefaultTab").addEventListener("click", () => {
  showTab("imageTable", ["bitImageTable"], "imagesDefaultTab", [
    "imagesBitsTab",
  ]);
});
document.querySelector("#imagesBitsTab").addEventListener("click", () => {
  showTab("bitImageTable", ["imageTable"], "imagesBitsTab", [
    "imagesDefaultTab",
  ]);
});

document.querySelector("#soundsDefaultTab").addEventListener("click", () => {
  showTab("soundTable", ["bitSoundTable"], "soundsDefaultTab", [
    "soundsBitsTab",
  ]);
});
document.querySelector("#soundsBitsTab").addEventListener("click", () => {
  showTab("bitSoundTable", ["soundTable"], "soundsBitsTab", [
    "soundsDefaultTab",
  ]);
});

document.querySelectorAll(".windowBack").forEach((element) => {
...
function showTab(show, hide, select, deselect) {
  if (show == "soundTable") openSounds();
  else if (show == "bitSoundTable") openBitSounds();
...
function back() {
  if (!playingLarge && openPanelLarge) {
    ...
    if (
      currentPanelLarge.id == "statusWindow" &&
  ...
  } else if (panelStack.length > 0) showPanel(panelStack.pop(), false);
}
...
function showPanel(panel, stack) {
  if (!playing) {
    if (document.querySelector("#" + panel) != currentPanel) {
      ...
      if (stack == null) panelStack = [];

      if (stack == null || !stack) {
        document
          .querySelector("#sideBar")
          .querySelectorAll(".overlayButton")
          .forEach((element) => {
        ...
        if (panel == "bonkImages") {
          document
            .querySelector("#imagesButton")
            .querySelector(".overlayButton")
            .classList.add("buttonSelected");
          openImages();
        } else if (panel == "bonkSounds") {
          document
            .querySelector("#soundsButton")
            .querySelector(".overlayButton")
            .classList.add("buttonSelected");
          openSounds();
          openBitSounds();
      ...
      } else if (stack) panelStack.push(oldPanel.id);
...
function showPanelLarge(panel) {
  if (!playingLarge) {
    if (document.querySelector("#" + panel) != currentPanelLarge) {
      ...
      if (panel == "testBonks") openTestBonks();
...
function getRaidEmotes(_, data) {
  ...
  channelEmotes.open(
    "GET",
    "https://api.twitch.tv/helix/chat/emotes?broadcaster_id=" + data[0],
    true,
  );
...
function checkVersion() {
  ...
  versionRequest.open(
    "GET",
    "https://itch.io/api/1/x/wharf/latest?game_id=1387406&channel_name=win32",
    true,
  );
  versionRequest.send();
}
...
document.querySelector("#testBarrage").addEventListener("click", () => {
  ipcRenderer.send("barrage");
});
document.querySelector("#testSub").addEventListener("click", () => {
  ipcRenderer.send("sub");
});
document.querySelector("#testSubGift").addEventListener("click", () => {
  ipcRenderer.send("subGift");
});
document.querySelector("#testCharity").addEventListener("click", () => {
  ipcRenderer.send("charity");
});
document.querySelector("#testBits").addEventListener("click", () => {
  ipcRenderer.send("bits");
});
document.querySelector("#testFollow").addEventListener("click", () => {
  ipcRenderer.send("follow");
});
document.querySelector("#testEmote").addEventListener("click", () => {
  ipcRenderer.send("emote");
});
document.querySelector("#testRaid").addEventListener("click", () => {
  ipcRenderer.send("raid");
});

document.querySelector("#calibrateButton").addEventListener("click", () => {
  if (!cancelCalibrate) ipcRenderer.send("startCalibrate");
});
document.querySelector("#nextCalibrate").addEventListener("click", () => {
  ipcRenderer.send("nextCalibrate");
});
document.querySelector("#cancelCalibrate").addEventListener("click", () => {
  ipcRenderer.send("cancelCalibrate");
  back();
});
